...
void gdtInitialize()
{
    _gdtp.limit = sizeof(struct gdtEntry) * GDT_ENTRIES - 1; // Limit of GDT, ie. the size in memory of the GDT
    _gdtp.base = (uint32_t) &gdt; // Pointer to start of GDT in memory
    
    gdtSetGate(0, 0, 0, 0, 0); // NULL descriptor, index 0
    ...
    gdtSetGate(1, 0, 0xFFFFFFFF, 0b10011010, 0b11001111);

    // Index 2 denoting data segment
    // Otherwise the same as code segment
    gdtSetGate(2, 0, 0xFFFFFFFF, 0b10010010, 0b11001111); 
